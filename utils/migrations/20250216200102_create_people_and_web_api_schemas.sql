...
-- web_api contains both crowd apps and api tokens so is also critical to keep secure
CREATE SCHEMA IF NOT EXISTS web_api;
-- We will create the tables in the following order
-- 1. web_api.stream_endpoints
--
-- web_api.stream_endpoints
CREATE TABLE IF NOT EXISTS web_api.stream_endpoints(
    endpoint_id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name text NOT NULL,
    ...
    CONSTRAINT stream_endpoints_application_name_uindex UNIQUE (application, name),
    CHECK ((end_valid IS NULL) OR (start_valid IS NULL) OR (end_valid > start_valid))
);

-- +goose Down

DROP SCHEMA people CASCADE;
DROP SCHEMA web_api CASCADE;
